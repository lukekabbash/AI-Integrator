...
| Claude Code | `claude-fable-5`, `claude-opus-5`, `claude-sonnet-5`, and `claude-haiku-4-5` where the user's Claude Code surface exposes them | User-owned `claude -p` structured CLI; `--model` plus Claude Code's `/effort` control; no AI Integrator account/login path | Do not store raw `thinking`/hidden chain-of-thought events; only retain provider-labeled summaries or observable final/tool activity |
